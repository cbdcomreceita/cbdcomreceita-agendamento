 ...
 * once the mode is on — not just the NexTalk token. Some of these
 * (signing keys, callback base URL) never cause a throw at publish time —
 * a missing signing key only breaks the QStash → route round trip
 * asynchronously, as a silent 401 nobody would otherwise notice — so this
 * is checked proactively in confirm-booking.ts before attempting to
 * publish, instead of waiting for a real webhook attempt to fail quietly.
 */
